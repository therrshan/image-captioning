...
def train_val_split(caption_data, train_size=0.7, shuffle=False):
...
'''We'll use the TextVectorization layer to vectorize the text data, that is to say, 
to turn the original strings into integer sequences where each integer represents the index 
of a word in a vocabulary. We will use a custom string standardization scheme 
(strip punctuation characters except < and >) and the default splitting scheme (split on whitespace).'''
...
caption_model.save_weights('/content/model_weights_inception', save_format='tf')
print("saved")